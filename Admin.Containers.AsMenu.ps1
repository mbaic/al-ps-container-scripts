...
# Suppress debugging context and progress for clean execution in VS Code
...
function Show-Menu {
    ...
    $title = "BC Container Admin"
    $padding = [math]::Floor((71 - $title.Length) / 2)
    $titleLine = "║" + (" " * $padding) + $title + (" " * (71 - $padding - $title.Length)) + "║"
    Write-Host $titleLine -ForegroundColor Green

    Write-Host "║                                                                       ║" -ForegroundColor Cyan
    Write-Host "╚═══════════════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "Current Container:" -ForegroundColor Cyan
    Write-Host ("  • {0}" -f $containerName) -ForegroundColor Green
    Write-Host ""

    Write-Host "Operations:" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "  Container Management:" -ForegroundColor Gray
    ...
    Write-Host ""
    Write-Host "  Configuration & Cleanup:" -ForegroundColor Gray
    Write-Host "    [5] Remove Container & Cleanup Cache" -ForegroundColor Red
    Write-Host "    [6] Import License" -ForegroundColor White
    Write-Host ""
    Write-Host "  Application Management:" -ForegroundColor Gray
    Write-Host "    [7] Show Non-Microsoft Apps" -ForegroundColor White
    Write-Host "    [8] Uninstall/Unpublish Non-Microsoft Apps" -ForegroundColor Red
    Write-Host "    [9] Install Apps from Folder" -ForegroundColor White
    Write-Host ""
...
function Restart-Container {
    ...
    if ($container) {
        ...
        Write-Host ("  Container{0} {1}" -f ("." * (25 - "Container".Length)), "✓ Restarted") -ForegroundColor Green
    }
    else {
...
function Stop-Container {
    ...
    if ($container) {
        ...
        Write-Host ""
        Write-Host "Result:" -ForegroundColor Cyan
        Write-Host ("  Container{0} {1}" -f ("." * (25 - "Container".Length)), "✓ Stopped") -ForegroundColor Green
    }
    else {
        Write-Host "[!] Container '$containerName' not found" -ForegroundColor Red
    }
...
function Show-ContainerLogs {
    ...
    if ($container) {
        Write-Host "How many lines to display? (default: 50)" -ForegroundColor Cyan
        $lines = Read-Host "Lines"
        if ([string]::IsNullOrWhiteSpace($lines)) { $lines = 50 }

        Write-Host ""
        Write-Host "[→] Fetching last $lines lines from $containerName..." -ForegroundColor Gray
        Write-Host ""
...
function Remove-ContainerWithConfirmation {
    ...
    Write-Host ("  Container name{0} {1}" -f ("." * (22 - "Container name".Length)), $containerName) -ForegroundColor White
    Write-Host ""
    ...
    if ($confirm -eq 'YES') {
        ...
        if ($container) {
            ...
            Write-Host ""
            Write-Host "Result:" -ForegroundColor Cyan
            Write-Host ("  Container removed{0} {1}" -f ("." * (18 - "Container removed".Length)), "✓") -ForegroundColor Green
            Write-Host ("  Cache flushed{0} {1}" -f ("." * (23 - "Cache flushed".Length)), "✓") -ForegroundColor Green
        }
        else {
            Write-Host "[!] Container '$containerName' not found" -ForegroundColor Red
        }
...
function Import-License {
    ...
    $useDefault = Read-Host "Use default path? (Y/N, default: Y)"
    ...
    if ($container) {
        try {
            ...
            Write-Host ""
            Write-Host "Result:" -ForegroundColor Cyan
            Write-Host ("  License imported{0} {1}" -f ("." * (19 - "License imported".Length)), "✓") -ForegroundColor Green
            Write-Host ("  Container{0} {1}" -f ("." * (25 - "Container".Length)), "Ready") -ForegroundColor Cyan
        }
        catch {
    ...
    if ($container) {
        try {
            ...
            if ($apps) {
                ...
                Write-Host ""
                Write-Host "Result:" -ForegroundColor Cyan
                Write-Host ("  Apps found{0} {1}" -f ("." * (24 - "Apps found".Length)), $appCount) -ForegroundColor Green
                Write-Host ""
                ...
                Write-Host ""
                Write-Host "Result:" -ForegroundColor Cyan
                Write-Host ("  Apps found{0} {1}" -f ("." * (24 - "Apps found".Length)), "0") -ForegroundColor Yellow
            }
        }
        catch {
            Write-Host "[!] Failed to retrieve app information: $($_.Exception.Message)" -ForegroundColor Red
...
function Uninstall-NonMicrosoftApps {
    ...
    try {
        ...
        Write-Host ("  Processed{0} {1}" -f ("." * (25 - "Processed".Length)), $selectedApps.Count) -ForegroundColor White
        Write-Host ("  Successful{0} {1}" -f ("." * (24 - "Successful".Length)), $successCount) -ForegroundColor Green
        Write-Host ("  Failed{0} {1}" -f ("." * (29 - "Failed".Length)), $failCount) -ForegroundColor $(if ($failCount -gt 0) { "Red" } else { "Gray" })
        Write-Host ""
        ...
        try {
            ...
            if ($remainingApps) {
                ...
                Write-Host "Remaining Apps:" -ForegroundColor Yellow
                Write-Host ("  Still in container{0} {1}" -f ("." * (18 - "Still in container".Length)), $remainingCount) -ForegroundColor Yellow
                Write-Host ""
                ...
                Write-Host ""
                Write-Host "Result:" -ForegroundColor Cyan
                Write-Host ("  Remaining apps{0} {1}" -f ("." * (21 - "Remaining apps".Length)), "0 ✓") -ForegroundColor Green
            }
        }
        catch {
...
function Install-AppsFromFolder {
    ...
    try {
        ...
        Write-Host ("  Container{0} {1}" -f ("." * (30 - "Container".Length)), $containerName) -ForegroundColor White
        Write-Host ("  Unpublish old{0} {1}" -f ("." * (26 - "Unpublish old".Length)), $(if ($unpublishOld) { 'Yes' } else { 'No' })) -ForegroundColor White
        Write-Host ""
        ...
        Write-Host ("  Total apps{0} {1}" -f ("." * (30 - "Total apps".Length)), $totalApps) -ForegroundColor White
        Write-Host ("  Successful{0} {1}" -f ("." * (29 - "Successful".Length)), $successCount) -ForegroundColor Green
        Write-Host ("    - New installs{0} {1}" -f ("." * (25 - "New installs".Length)), $newInstallCount) -ForegroundColor Cyan
        Write-Host ("    - Upgrades{0} {1}" -f ("." * (28 - "Upgrades".Length)), $upgradeCount) -ForegroundColor Cyan
        Write-Host ("  Skipped{0} {1}" -f ("." * (32 - "Skipped".Length)), $skipCount) -ForegroundColor Gray
        Write-Host ("  Failed{0} {1}" -f ("." * (33 - "Failed".Length)), $failCount) -ForegroundColor $(if ($failCount -gt 0) { "Red" } else { "Gray" })
        if ($unpublishOld -and $oldVersionsToUnpublish.Count -gt 0) {
            Write-Host ("  Old versions unpublished{0} {1}" -f ("." * (16 - "Old versions unpublished".Length)), $oldVersionsToUnpublish.Count) -ForegroundColor Yellow
        }
        Write-Host ""
...
function Switch-Container {
    ...
    if ($availableContainers.Count -gt 0) {
        for ($i = 0; $i -lt $availableContainers.Count; $i++) {
            if ($availableContainers[$i] -eq $containerName) {
                Write-Host ("  [$($i + 1)] {0,-30} ◄ Current" -f $availableContainers[$i]) -ForegroundColor Green
            }
            else {
                Write-Host ("  [$($i + 1)] {0,-30}" -f $availableContainers[$i]) -ForegroundColor White
            }
        }
    }
    else {
    ...
    Write-Host ""
    Write-Host "Result:" -ForegroundColor Cyan
    Write-Host ("  New container{0} {1}" -f ("." * (22 - "New container".Length)), $newContainerName) -ForegroundColor Green
    Write-Host ("  Status{0} {1}" -f ("." * (31 - "Status".Length)), "✓ Switched") -ForegroundColor Green
    Write-Host ""
    pause
}
...
do {
    ...
    switch ($choice.ToUpper()) {
        ...
        '6' { Import-License }
        '7' { Show-NonMicrosoftApps }
        '8' { Uninstall-NonMicrosoftApps }
        '9' { Install-AppsFromFolder }
        'S' { Switch-Container }
        ...
        'Q' {
            ...
            Write-Host "╔═══════════════════════════════════════════════════════════════════════╗" -ForegroundColor Cyan
            Write-Host "║                                                                       ║" -ForegroundColor Cyan
            Write-Host "║                   Thank you for using BC Container Admin             ║" -ForegroundColor Green
            Write-Host "║                                                                       ║" -ForegroundColor Cyan
            Write-Host "╚═══════════════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
            Write-Host ""
            exit
